...
public class PersonSQL {
  ...
  private static String jdbc_friver = "org.sqlite.JDBC";
  private static String db_file = "jdbc:sqlite:src/com.hauge.contact/rsc/db/person.db";
  ...
  private static void kobleOpp() {
    try {
      Class.forName(jdbc_friver);
      conn = DriverManager.getConnection(db_file);  
    }
    catch (ClassNotFoundException e) {
      feil("Fant ikke JDBC-driver " + jdbc_friver + "\n" + e.toString());
    }
    catch (SQLException e) {
      feil("Oppkobling til databasen " + db_file + " feilet.\n" + e.toString());
    }
  } 
